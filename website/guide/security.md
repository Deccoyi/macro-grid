...
- **Plugins by other authors are not reviewed.** A C# plugin can read your files, start programs and connect to the internet and send data; Macro Grid cannot limit or check that. A JavaScript plugin can only send web requests to the addresses it lists, and its permission says whether each is on this PC, your local network or the internet.
