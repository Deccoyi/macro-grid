...
export function ActionPicker({ actions, onPick, renderTrigger }: ActionPickerProps) {
  ...
  const items = usePickerFilter(
    ...
    picker.query,
    (a) => a.category,
    (a, q) => catalogText.actionName(a).toLowerCase().includes(q) || catalogText.actionDescription(a).toLowerCase().includes(q),
  );
          ...
          {items.map((a) => (
            ...
            >
              ...
              <span style={{ minWidth: 0 }}>
                <div style={{ fontSize: 12.5 }}>{catalogText.actionName(a)}</div>
                {catalogText.actionDescription(a) && <div style={{ fontSize: 11, color: "var(--ms-text-secondary)" }}>{catalogText.actionDescription(a)}</div>}
              </span>
            </button>
          ))}
        </PickerShell>
