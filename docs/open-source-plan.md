# Open-sourcing the three Macro Station repos

## Context

`macro-station` (server + editor), `macro-station-client` (Android client) and `macro-station-plugin` (plugins + authoring
...
`macro-station/docs/open-source-plan.md` (English, per project rules).
...
- All three repos: `dev` is the working branch. `macro-station` and `-client` also have `main`. **`macro-station-plugin` has only `dev`** (its `origin/HEAD` points at `dev`).
- **No GitHub Actions / CI** anywhere. No `SECURITY.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, issue/PR templates.
- **`macro-station-plugin` has no `LICENSE` file.**
- The plugin repo's C# projects reference the SDK by relative path (`..\..\..\macro-station\src\MacroStation.Plugin.Abstractions`). A public plugin author cannot build that; the SDK is not on NuGet. This blocks the "hello world C# plugin" tutorial.
- `docs/release.md`: the installer script (`MacroStation.iss`) has **never been compiled or tested on a clean PC**. No signed release APK flow verified. No GitHub Release exists.
- README security note in the server repo is stale ("no pairing/authentication layer") while PIN + token pairing exists.
- Git history: 90 commits across 3 repos (checked with `git log --all`). Three author identities are present: the project handle with a personal email (most commits), one commit with a real-name variant, and 11 plugin-repo commits with an employer email. All of them must be replaced (see O2). Repos are small (<3 MB), no secrets / keystores / `.env` found in tracked files by grep.
- An employer network/domain reference sits in `docs/agent-notes.md` of the server and client repos, in the current files and in many past commits. A machine IP sits in `macro-station-plugin/docs/handoff-2026-09-23.md`. `macro-station/docs/plan.md` and `docs/agent-notes.md` are largely Turkish internal notes with stale "empty folder / will do" text.
- PLC Icons: only about 20-25 icons are original AI-generated art; the rest come from the Lucide icon library (ISC licence; its licence text and copyright notice must ship with the pack and be listed in `THIRD_PARTY_NOTICES.md`). The pack README states neither. App icons are AI-generated too. Disclosure needed, see O3.
- `macro-station/THIRD_PARTY_NOTICES.md` covers only direct deps and does not list Jint, the plugin repo's deps, or Android/Capacitor deps (client has its own file, not reviewed here).
...
| # | Gate | Who verifies |
|---|---|---|
| G1 | Identity scrub done (O2): history rewritten, repos recreated, scan finds no employer name, old email or real name anywhere in tree or history | Claude scans, owner executes |
...
1. Add missing `LICENSE` (MIT, same as others) to `macro-station-plugin`; per-plugin license note where needed.
...
3. Make `main` the default and `dev` the integration branch in `macro-station-plugin` (create `main`); document the branching model in CONTRIBUTING.
...
7. Complete `THIRD_PARTY_NOTICES.md` for each repo by reading csproj / package.json / gradle files (Jint, Capacitor, MLKit, etc.); no guessing.
8. Re-run secret/PII scan (tracked files and full history, including deleted `_backup/` content) and report.
...
11. Prepare NuGet packaging of `MacroStation.Plugin.Abstractions` (`PackageId`, metadata, README, symbol package, `Version` 0.3.0) and switch plugin csproj files to a `PackageReference` with a documented local-feed/ProjectReference fallback for developers working on both repos. The owner does the actual `nuget push`.
12. Enable Dependabot config (`dependabot.yml`) for nuget, npm, gradle, github-actions.

**C. Documentation site (in `macro-station-plugin`, deployed with GitHub Pages)**
13. Tool: VitePress (Node/Vite already used in the project, Markdown-native, built-in search, dark mode). Site source in `macro-station-plugin/website/`; deployed by a `pages.yml` workflow (upload-pages-artifact + deploy-pages). Base path `/macro-station-plugin/` until a custom domain exists.
14. Content structure:
    - Introduction: what Macro Station is, architecture (server, client, plugins), alpha status and disclaimer
...
- **O2 Identity scrub (decided: full scrub, history is rewritten).** Nothing that names the employer or the old personal email or real name may remain in any repo, commit metadata or file, in any past commit.
  1. Create the new mailbox and add it to the GitHub account (Settings > Emails); tick "Keep my email addresses private" and "Block command line pushes that expose my email". Use the account's `<id>+<handle>@users.noreply.github.com` address as the author email in all rewritten commits, so the new mailbox itself is never exposed either. Set global git config to it on every machine that commits.
  2. Claude prepares (in the scratchpad, on fresh mirror clones, never on the working repos) a `git filter-repo` run per repo: a `--mailmap` mapping all old identities (personal email, employer email, real-name variants) to the handle plus the noreply address, and `--replace-text` rules that strip the employer domain and personal strings from file contents in ALL commits. Then re-scan history for every old string, including the local part of each old email. The exact strings live only in the scratchpad rules file, never in the repo.
...
- **O5 Publish packages:** create a nuget.org account/API key and push `MacroStation.Plugin.Abstractions` `0.3.0`. Credentials stay with the owner.
...
2. (Settled) Docs site uses the default `<handle>.github.io/macro-station-plugin/` address.
