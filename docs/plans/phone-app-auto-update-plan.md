...
Status: **approved by the owner** (2026-09-25), with the decisions under "Decisions" at the end. Phases 1 to 4 are approved to build in order; phase 5 is asked for separately. Progress (2026-09-25, nothing committed yet): phases 1 (release process), 2 (native plugin) and 3 (web side: feed, policy, state, schedule, update screen, drawer row and dot, full-screen Settings page, texts, 28 new unit tests) are written in `macro-grid-client`. A real self-update `0.1.1 → 0.9.0` was run on a phone through the app's own screens (made-up release list in a test build, the pairing survived the update, the downloaded file was removed afterwards). Phase 4 (legal text, docs, websites) is next. As built: the SHA-256 is computed by reading the finished file once (not while streaming), and `cleanUp` takes the one version to keep (`keep`, null for none) because the web side decides which version that is.
