...
| 5 | **Security hardening:** encrypted connection on the local network, secret storage for plugins, "remove my data" on uninstall | `macro-grid`, `macro-grid-client`, `macro-grid-plugin` | [security-hardening-plan.md](security-hardening-plan.md) | What the risk assessment ([../design/security-risk-assessment.md](../design/security-risk-assessment.md)) leaves open. Waiting for the owner's decision. |
| 6 | Roadmap "Next": editor keyboard shortcuts with undo/redo, a branded installer (logo and colors), the `plugin-html` widget, the `web` widget, an async host API for JavaScript plugins | `macro-grid`, `macro-grid-client` (the `web` widget) | [roadmap.md](../roadmap.md) | Larger, no user is blocked on them. |
