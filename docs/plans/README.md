...
| 1 | **Priority: publish the SDK package only when the SDK changes.** The server and the SDK keep one version number, but the NuGet package is published only when `src/MacroGrid.Plugin.Abstractions` differs from the previous server tag, so a release that does not touch the SDK does not create a new package. Add a check that fails the build when a public SDK member is removed or changed inside a MAJOR (package validation against the last published package, and a test that loads a plugin built for an older SDK on the new server). | `macro-grid` | none yet: decision only, notes in [roadmap.md](../roadmap.md) ("Next") | The owner finds a new package for an unchanged SDK pointless (1.0.1 was such a release). Agreed in principle, to be discussed before it is built. |
...
| 3 | **Auto-update of the phone app** (release-signed APK on every release, check GitHub, download, verify, Android's installer) | `macro-grid-client` (all code), `macro-grid` (website pages) | [phone-app-auto-update-plan.md](phone-app-auto-update-plan.md) | Waiting for the owner's approval. Reuses the server's update rules; its first phase fixes the release signing, which every later client release needs. |
| 4 | **Issues and Discussions:** a label system, forms plus a plain-text report to fill in, sorting by area on its own, Discussions for questions and ideas | `macro-grid`, `macro-grid-client`, `macro-grid-plugin` | [issues-and-discussions-plan.md](issues-and-discussions-plan.md) | Small and no code, but it changes how people reach the project. Local files are ready; the GitHub steps (Discussions, labels) wait for the owner. |
| 5 | **SDK 0.4.0 and the sound plugin** | `macro-grid`, then `macro-grid-plugin` | [sound-plugin-plan.md](sound-plugin-plan.md) (phase A here, phase B in the plugin repository) | Needs new settings field kinds, so it changes the SDK and the editor forms first. Add the plugin logo field in the same SDK bump (roadmap, "Next"). |
| 6 | Roadmap "Next": an encrypted connection (TLS), editor keyboard shortcuts with undo/redo, a branded installer (logo and colors), the `plugin-html` widget, the `web` widget, an async host API for JavaScript plugins | `macro-grid`, `macro-grid-client` (the `web` widget) | [roadmap.md](../roadmap.md) | Larger, no user is blocked on them. |
