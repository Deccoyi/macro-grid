...
5. **Client (Aşama 5 tamam, gerçek cihaz/USB testi hariç, ayrı repo):** Eşleştirme — PIN ve QR tamam, mDNS henüz yok. Cihaz bazında profil ataması tamam (Eşleştirme penceresinde cihaz başına profil `<select>`'i, atanmamışsa ilk profile düşülüyor). Profil drawer'ı ve client'tan `profile.switch` tamam. Sayfa geçişi client'tan tetikleniyor (yatay swipe → `page.next`/`page.prev`). Keep-awake + yeniden bağlanma + son-layout önbelleği tamam. Kiosk modu (immersive/tam ekran) ve yön kilidi tamam, drawer içinden Ayarlar paneliyle açılıp kapanıyor. Tarayıcı client (`webclient/`, aynı grid mantığı, server'dan `/deck/` altında servis ediliyor) tamam. **Kalan:** gerçek cihazda USB debug testi.
6. **Plugin'ler:** Soundboard, OBS, IconPacks, WebView/chat (iframe ve native overlay). Ses (master volume slider/mute) plan'da plugin olarak listelenmişti ama built-in aksiyon olarak Core'a taşındı (`core.setVolume`/`core.setMute`/`core.toggleMute`, `system.audio.master`/`system.audio.muted` değişkenleri) — bkz. Aşama 2. OBS API'sinden (obs-websocket v5) çıkarılan plugin sistemi gereksinimleri `agent-notes.md`'de not edildi; bir OBS-plugin taslağı (WebSocket client, kimlik doğrulama, aksiyonlar) `_backup/obs-plugin-reference/`'da hazır bekliyor, loader kurulunca OBS plugin'inin başlangıç noktası olarak kullanılacak.
