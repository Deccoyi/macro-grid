...
- **Profil seçimi (drawer, butondan aksiyon, ve artık aktif pencereye göre otomatik geçiş de tamam):** Kenardan swipe (+ her zaman görünen ince tutamaç) ile açılan **profil drawer'ı tamam** (`profiles.list`/`profile.change` mesajları, `SessionDeviceController.SwitchProfileAsync`). Bir butondan da `core.profile` aksiyonuyla (`ProfileAction`, bkz. `PageActions.cs`) değiştirilebiliyor. **Otomatik profil geçişi** (bkz. `docs/auto-profile-switch.md`): her profil `AppMatches` (process adı + opsiyonel başlık filtresi) taşıyabiliyor, `MacroStation.Windows.ForegroundWindowMonitor` (`SetWinEventHook(EVENT_SYSTEM_FOREGROUND)`, event tabanlı — polling yok) ile `AutoProfileSwitcher` (Core, `IHostedService`) bunu her oturumun kendi `AutoSwitchState` yığınına uyguluyor: tanımlı bir pencere öne gelince o profile geçiliyor, kapanınca bir önceki (elle seçilen ya da başka bir kuralın) profile dönülüyor, yığın boşalırsa `ProfileResolver.ResolveDefault` (cihaz ataması → `AppPreferences.DefaultProfileId` → ilk profil) devreye giriyor. Her cihaz kendi açar (`PairedDevice.FollowActiveWindow`, Eşleştirme penceresinde anahtar); drawer'da bir kilit (`PairedDevice.AutoSwitchLocked`, `profile.lock` mesajı) otomatik geçişi durdurup elle geçişi açık bırakıyor. Editörde profil panelindeki pencere ikonuyla açılan bir bölümden kurallar eklenip çalışan uygulamalardan (`GET /api/system/windows`) seçilebiliyor ya da elle exe adı girilebiliyor; Tercihler'de "Profiller" kategorisinde varsayılan profil seçilebiliyor. **Gerçek OS ön-plan-penceresi olayıyla uçtan uca doğrulanmadı** — `AutoSwitchState`'in yığın/kilit/varsayılan mantığı unit test'lerle (`AutoSwitchStateTests`) kapsanıyor, `/api/system/windows` gerçek pencere listesini doğru döndürdüğü canlı doğrulandı, ama `SetWinEventHook` callback'inin gerçek bir foreground değişikliğini yakalayıp yakalamadığı henüz test edilmedi (bkz. handoff notu).
