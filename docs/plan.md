...
1. **Stage 9:** widget text/icon dynamization (small, scope is clear).
2. **Stage 8:** packaging (`.msprofile` import/export, single-file publish + installer, signed APK).
3. **Stage 7:** JS plugin runtime (Jint sandbox, permissions, `plugin-html` bridge). Biggest item.
4. Loose ends: the browser client (`webclient/`) does not announce `assets`/`layout.patch` yet; icons chosen through dynamic rules or pushed in `widget.state` styles are not externalized yet.
