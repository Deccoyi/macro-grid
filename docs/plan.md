...
| Windows Güvenlik Duvarı | — | İlk çalıştırmada 9820 portu için "Özel ağ" izni verilecek (kullanıcı onaylar). Wi-Fi ağ profili "Özel" olmalı, "Genel" profilde bağlantı engellenir |
...
├── plugins/                        (henüz yok) Obs/, Audio/, IconPacks/
└── tests/
```
...
- **Profil seçimi (drawer, butondan aksiyon, ve artık aktif pencereye göre otomatik geçiş de tamam):** Kenardan swipe (+ her zaman görünen ince tutamaç) ile açılan **profil drawer'ı tamam** (`profiles.list`/`profile.change` mesajları, `SessionDeviceController.SwitchProfileAsync`). Bir butondan da `core.profile` aksiyonuyla (`ProfileAction`, bkz. `PageActions.cs`) değiştirilebiliyor. **Otomatik profil geçişi** (bkz. `docs/done/auto-profile-switch.md`): her profil `AppMatches` (process adı + opsiyonel başlık filtresi) taşıyabiliyor, `MacroStation.Windows.ForegroundWindowMonitor` (`SetWinEventHook(EVENT_SYSTEM_FOREGROUND)`, event tabanlı — polling yok) ile `AutoProfileSwitcher` (Core, `IHostedService`) bunu her oturumun kendi `AutoSwitchState` yığınına uyguluyor: tanımlı bir pencere öne gelince o profile geçiliyor, tanımsız bir pencere öne gelince (odak kaybı) ya da uygulama kapanınca elle seçilen profile dönülüyor, yığın boşalırsa `ProfileResolver.ResolveDefault` (cihaz ataması → `AppPreferences.DefaultProfileId` → ilk profil) devreye giriyor. Her cihaz kendi açar (`PairedDevice.FollowActiveWindow`, Eşleştirme penceresinde anahtar); drawer'da bir kilit (`PairedDevice.AutoSwitchLocked`, `profile.lock` mesajı) otomatik geçişi durdurup elle geçişi açık bırakıyor. Editörde profil panelindeki pencere ikonuyla açılan bir bölümden kurallar eklenip çalışan uygulamalardan (`GET /api/system/windows`) seçilebiliyor ya da elle exe adı girilebiliyor; Tercihler'de "Profiller" kategorisinde varsayılan profil seçilebiliyor. Gerçek OS ön-plan olaylarıyla uçtan uca doğrulandı (kural penceresi öne gelince profile geçiş, başka pencereye odak geçince elle seçilen profile dönüş, kilit). Drawer kilidi kilit ikonlu bir switch.

## Mimari
- **Haberleşme:** Kestrel üzerinde düz WebSocket ve JSON mesajlar, port 9820.
  - Server → client: `welcome` (+ pairing token), `layout.full`, `layout.patch` (a saved edit reaches a client as only the changed widgets; the client announces `layout.patch` in `hello.capabilities`, see `docs/done/layout-patch-and-assets.md`), `widget.state` (render edilmiş text, değer, stil override'ı), `profiles.list`, `asset` (large `data:` values such as icons are sent once per client as `asset:<hash>` references and cached there)
...
5. **Client (Aşama 5 tamamen bitti, ayrı repo):** Eşleştirme — PIN ve QR tamam. Cihaz bazında profil ataması tamam (Eşleştirme penceresinde cihaz başına profil `<select>`'i, atanmamışsa ilk profile düşülüyor). Profil drawer'ı ve client'tan `profile.switch` tamam. Sayfa geçişi client'tan tetikleniyor (yatay swipe → `page.next`/`page.prev`). Keep-awake + yeniden bağlanma + son-layout önbelleği tamam. Kiosk modu (immersive/tam ekran) ve yön kilidi tamam, drawer içinden Ayarlar paneliyle açılıp kapanıyor. Tarayıcı client (`webclient/`, aynı grid mantığı, server'dan `/deck/` altında servis ediliyor) tamam. **Gerçek cihazda USB debug testi de yapıldı (2026-09-23):** `adb install` ile debug APK gerçek bir Android telefona kuruldu, önceden eşleştirilmiş cihaz LAN üzerinden yeniden bağlandı, canlı değişken push'ı ve dokunma girişi doğrulandı — ayrıntı için `macro-station-client/docs/plan.md`.
6. **Plugin loader'ı tamam; ilk gerçek plugin (OBS, v0.2.0) yazıldı, editörde şema-tabanlı ayar arayüzü ve durum çubuğuyla birlikte.** `IPlugin`/`IPluginHost` (`MacroStation.Plugin.Abstractions`, SDK `0.3.0` — `IPluginHost` artık `DataDirectory`/`Log`/`RegisterSettingsPage`/`CreateStatusItem`/`RegisterIconPack` da veriyor), her plugin kendi izole `AssemblyLoadContext`'inde (`MacroStation.Core/Plugins/PluginLoadContext.cs`), `%AppData%/MacroStation/plugins/` klasör taraması + `plugin.json` doğrulama (`sdkVersion` caret aralığı, `minServerVersion`) `PluginManager` ile sunucu açılırken yükleniyor ve artık çalışırken de yüklenip kaldırılabiliyor (see below). Editörde "Eklentiler" penceresi gerçek listeyi (`GET /api/plugins`) gösteriyor ve "Klasörden Yükle…" ile native klasör seçme diyaloğu üzerinden bir plugin klasörünü `plugins/<id>/`'e kopyalıyor (it is loaded right away — **hot loading**: `PluginManager` also reloads and uninstalls a plugin while the server runs; a plugin's actions, variables, status items, settings page and icon packs are registered on load and taken back out on unload, its assembly context is unloaded and the DLLs are loaded from memory so they are never locked on disk; the editor's Plugins window has install, reload and remove, and the main editor refetches its action/variable/icon catalogs when it regains focus). JS/Jint çalıştırma motoru yok (Aşama 7), `kind:"js"` manifestolar tanınıyor ama "Uyumsuz" gösteriliyor. Ayrıntı ve plugin yazma rehberi: `macro-station-plugins/docs/plugin-authoring.md`. Ses (master volume slider/mute) plan'da plugin olarak listelenmişti ama built-in aksiyon olarak Core'a taşındı (`core.setVolume`/`core.setMute`/`core.toggleMute`, `system.audio.master`/`system.audio.muted` değişkenleri) — bkz. Aşama 2.

   Editör artık genel (plugin'e özel olmayan) bir **schema-driven form altyapısı** sağlıyor: bir action handler `IActionDescriptor` (Category/Description/Icon/Fields) uygularsa ve/veya plugin bir `IPluginSettingsPage` kaydederse, editör `SchemaForm.tsx` ile formu otomatik çizer — elle React formu yazmaya gerek kalmıyor. Dinamik dropdown'lar `IOptionsSource` ile (`POST /api/actions/{type}/options/{sourceId}`, `POST /api/plugins/{id}/settings/options/{sourceId}`) sağlanıyor. Kategorize/aranabilir bir **aksiyon seçici** (`ActionPicker.tsx`) eski düz `<select>`'in yerini aldı. Pencere genelinde bir **durum çubuğu** (`StatusBar.tsx`, `IPluginHost.CreateStatusItem`/`PluginStatusRegistry`) sol tarafta sunucu sürümü/bağlı cihaz sayısını, sağ tarafta plugin'lerin kendi durumunu gösteriyor; bir plugin durumuna tıklamak onun ayar penceresini açıyor (`PluginSettingsWindow.tsx`, `POST /api/windows/plugin-settings/{id}`).

   **OBS plugin'i** (`macro-station-plugins/OBS/`, önceki `_backup/obs-plugin-reference/`'daki taslak temel alındı, v0.2.0) obs-websocket v5'e bağlanıyor: doğru event subscription bitleri, bağlantı/istek zaman aşımları, `RequestBatch` ile tek frame'de toplu durum sorgusu (yayın/kayıt açıkken 1s, değilse 5s), kapanış koduna göre (4009/4010/4012) şifre hatasında sonsuz yeniden deneme yerine ayar değişene kadar bekleme, ve bir `ObsState` önbelleği (sahneler, ses girişleri, gruplar dahil sahne öğeleri, profiller, geçişler) — tüm dropdown'lar ve `obs.*` değişkenleri (~45 adet, sahne/yayın/kayıt/istatistik/dinamik input-item değişkenleri dahil) buradan servis ediliyor, canlı OBS round-trip'i beklemiyor. ~20 aksiyon (sahne/geçiş/profil, yayın/kayıt/sanal kamera/tekrar arabelleği, ses seviyesi/mute, sahne öğesi görünürlüğü, metin kaynağı) hepsi `IActionDescriptor`+`IOptionsSource` ile schema-driven form kullanıyor; artık silinmiş bir hedefe (ör. kaldırılmış bir sahneye) yönlendirilmiş bir aksiyon çalıştırıldığında sessizce no-op olmak yerine açık hata veriyor. Bağlantı ayarları artık `ObsSettingsPage : IPluginSettingsPage` ile genel şema formunda düzenleniyor (host'a özel OBS kodu editörden tamamen kaldırıldı). Sahte bir obs-websocket sunucusuna (`FakeObsServer`, `HttpListener` tabanlı) karşı çalışan xUnit entegrasyon test paketi eklendi (`macro-station-plugins/OBS/tests/MacroStation.Plugin.Obs.Tests/`) — handshake, 4009 şifre hatası, zaman aşımı, ani kopma, `ExitStarted`, `InputRemoved` değişken temizliği. **İkon paketleri:** `IIconPackSource`/`RegisterIconPack` ile host ve editör desteği tamam (ikon seçicide plugin paketleri ayrı kategori); ilk paket PLC İkonları (`macro-station-plugins/PLCIcons/`, 27 ladder-logic ikonu).
7. **JS plugin runtime:** Jint sandbox'ı, izin sistemi, ayar arayüzü. `plugin-html` widget köprüsü.
8. **Packaging (done, see `docs/release.md`):** `.msprofile` export/import (zip with `profile.json` and a manifest naming the plugins the profile needs; import warns about missing plugins, refuses a newer package format, still accepts the older plain JSON). Server: `scripts/publish.ps1` builds a self-contained single-file `MacroStation.exe` (about 62 MB, verified to start and serve the editor) and `installer/MacroStation.iss` + `installer/build-installer.ps1` wrap it in an Inno Setup installer (start menu, optional desktop shortcut, optional start with Windows, firewall rule for private networks, user data kept on uninstall). **The installer script has not been compiled yet: Inno Setup is not installed on the development machine.** Client (`macro-station-client`): the release APK is built by `scripts/build-release-apk.ps1`, version taken from `package.json`, signed from `android/keystore.properties` (verified with a throwaway keystore; the real keystore must be created by the owner and kept safe, see the client repo's `docs/release.md`). Not done: code signing of the installer, an in-app update check, Play Store publishing.
9. **Widget text/icon dynamization (done):** the button, toggle and label text and icon fields got the same dynamize ("Build logic") lightning-bolt as the colors. The `text` rule results are free text that may contain `{variables}` (`DynamicText`, resolved by `WidgetStateService`, which now always pushes the text of a dynamized widget so a client drops a stale rule result); the `style.icon` rule results are icons picked with the normal icon picker (baked with the widget's text color) and travel as a live style value, as an `asset:<hash>` reference for clients with the `assets` capability. An empty case result means no icon. The editor canvas previews both. Both renderer copies accept a live `icon`.

### Remaining order (as of 2026-09-23)
Done and verified against the running server: `layout.patch`, cached assets, plugin hot loading (see `docs/done/layout-patch-and-assets.md`).
Also done: stage 9 (widget text/icon dynamization) and stage 8 (packaging).
1. **Stage 7:** JS plugin runtime (Jint sandbox, permissions, `plugin-html` bridge). The last numbered stage and the biggest item.
2. Before a first public release: compile and test the installer on a clean PC (Inno Setup), create the real APK keystore, decide on code signing.
3. Loose end: the browser client (`webclient/`) does not announce `assets`/`layout.patch` yet.
...
- Uçtan uca test: 4x3 grid'de 2x2 bir buton, bir ses slider'ı ve bir sohbet web widget'ı oluşturulur, ayrıca ikinci bir sayfa ve ona geçiş butonu eklenir. İki cihaz (emülatör + tarayıcı) eşleştirilir ve her birine farklı profil atanır. Kontrol edilecekler:
  ...
  - Aktif pencereye göre otomatik profil geçişi: kural penceresi öne gelince profil değişiyor, başka pencereye odak geçince elle seçilen profile dönüyor, drawer kilidi geçişi durduruyor (gerçek makinede doğrulandı)
